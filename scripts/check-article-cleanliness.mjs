...
async function inspectQualifiedArticles() {
  ...
  const articleHrefs = await page
    .locator('.bm-research-lead__title a, .bm-research-library__copy h3 a')
    .evaluateAll((nodes) => [...new Set(nodes.map((node) => node.href).filter(Boolean))]);

  if (!articleHrefs.length) {
  ...
  for (const [index, articleHref] of articleHrefs.slice(0, 12).entries()) {
    ...
    const metrics = await page.evaluate(() => {
      ...
      const chatgptTrackedLinks = body
        ? [...body.querySelectorAll('a[href]')]
            .map((node) => node.href)
            .filter((href) => /[?&]utm_source=chatgpt\.com(?:&|$)/i.test(href))
        : [];
    ...
    if (!metrics.description || metrics.description.length < 60) fail(`Qualified article meta description is missing/too thin (${metrics.description.length} chars): ${articleUrl}`);
    if (/noindex/.test(metrics.robots) && /bitmomo\.id$/i.test(new URL(baseUrl).hostname)) fail(`Production qualified article is unexpectedly noindex: ${metrics.robots}`);
    if (!metrics.canonical) fail(`Qualified article has no canonical URL: ${articleUrl}`);
    if (!['MARKET RESEARCH', 'INTELLIGENCE SYSTEMS RESEARCH', 'AI & INTELLIGENCE SYSTEMS'].includes(metrics.eyebrow)) fail(`Qualified article has wrong eyebrow: ${metrics.eyebrow || 'missing'} (${articleUrl})`);
    if (metrics.bodyNewsletterNodes) fail(`Qualified article body contains ${metrics.bodyNewsletterNodes} newsletter/form node(s): ${articleUrl}`);
    if (metrics.bodyDisclaimers) fail(`Qualified article body contains ${metrics.bodyDisclaimers} legacy template disclaimer(s): ${articleUrl}`);
    if (metrics.legacyMarkers.length) fail(`Qualified article body contains legacy chrome text: ${metrics.legacyMarkers.join(' | ')} (${articleUrl})`);
    if (metrics.inlineTypography.length) fail(`Qualified article body contains inline typography overrides: ${JSON.stringify(metrics.inlineTypography.slice(0, 6))} (${articleUrl})`);
    if (metrics.minBodyParagraphSize !== null && metrics.minBodyParagraphSize < 15) fail(`Qualified article has paragraph/list text below 15px: ${metrics.minBodyParagraphSize}px (${articleUrl})`);

    if (metrics.chatgptTrackedLinks.length) {
      fail(`Qualified article contains ChatGPT tracking parameters in source links: ${articleUrl}`);
    }
    if (/^(?:excerpt\s*:)/i.test(metrics.bodyText) || /^(?:excerpt\s*:)/i.test(metrics.deckText)) {
      fail(`Qualified article exposes a literal "Excerpt:" authoring artifact: ${articleUrl}`);
    }
    if (/\[(?:…|\.\.\.)\]\s*$/.test(metrics.deckText)) {
      fail(`Qualified article deck contains an auto-excerpt truncation marker: ${articleUrl}`);
    }
    if (metrics.deckText && metrics.firstParagraph) {
      const deckComparable = compact(metrics.deckText.replace(/\[(?:…|\.\.\.)\]\s*$/, '')).toLowerCase();
      const paragraphComparable = compact(metrics.firstParagraph).toLowerCase();
      if (deckComparable.length >= 60 && paragraphComparable.startsWith(deckComparable.slice(0, Math.min(120, deckComparable.length)))) {
        fail(`Qualified article deck duplicates the opening paragraph: ${articleUrl}`);
      }
    }
