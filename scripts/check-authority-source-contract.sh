...
grep -q "RISET UTAMA" "${hub}"
grep -q "RISET TERBARU" "${hub}"
grep -q "STANDAR RISET" "${hub}"
grep -q "SISTEM AI & INTELLIGENCE" "${hub}"
grep -q "research_q" "${hub}"
grep -q "bm_visible_filters" "${hub}"
grep -q "bm_visible_desks" "${hub}"
grep -q "bm_visible_topics" "${hub}"
grep -q 'aria-label="Desk riset"' "${hub}"
grep -q "bm-research-filter--topics" "${hub}"
grep -Eq "TEMUAN UTAMA|RINGKASAN RISET" "${hub}"
grep -q "FIGUR RISET" "${hub}"
...
grep -q "Riset Pasar" "${footer}"
grep -q "Riset AI" "${footer}"
grep -q "Standar Riset" "${footer}"

library_line="$(grep -n "RISET TERBARU" "${hub}" | head -1 | cut -d: -f1)"
method_line="$(grep -n "STANDAR RISET" "${hub}" | tail -1 | cut -d: -f1)"
test "${library_line}" -lt "${method_line}"

grep -q "bm-research-filter" "${css}"
grep -q "bm-research-filter--topics" "${css}"
