...
$bm_research_query = null;
$bm_v3_active = function_exists( 'bitmomo_research_taxonomy_is_active' ) && bitmomo_research_taxonomy_is_active();

// Homepage Research shares the same institutional identity boundary as the
// Research Hub. V3 uses canonical desk/topic metadata; the legacy category/tag
// bridge remains available only until the audited migration activates V3.
if ( $bm_v3_active && function_exists( 'bitmomo_research_query_args' ) ) {
    $bm_query_args = bitmomo_research_query_args(
        'all',
        '',
        array(
            'posts_per_page' => 12,
...
if ( $bm_research_query instanceof WP_Query && $bm_research_query->have_posts() ) {
    while ( $bm_research_query->have_posts() && count( $bm_research_items ) < 3 ) {
        $bm_research_query->the_post();

        // This is the final authority boundary in both migration states. In V3
        // it requires exactly one Market Research desk plus the Riset container.
...
if ( ! $bm_research_items ) {
    unset( $bm_riset_term, $bm_riset_url, $bm_research_items, $bm_research_query, $bm_v3_active, $bm_query_args, $bm_home_priority_slugs, $bm_market_slugs, $bm_ai_lab_tag, $bm_reading_minutes );
    return;
}
?>
...
<?php unset( $bm_riset_term, $bm_riset_url, $bm_research_items, $bm_research_query, $bm_v3_active, $bm_query_args, $bm_home_priority_slugs, $bm_market_slugs, $bm_ai_lab_tag, $bm_reading_minutes, $bm_index, $bm_item ); ?>
