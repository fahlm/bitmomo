...
$bm_snapshot = class_exists( 'Bitmomo_Public_Intelligence_Adapter' )
    ...
    : null;
$bm_history = class_exists( 'Bitmomo_Public_Intelligence_Adapter' ) && method_exists( 'Bitmomo_Public_Intelligence_Adapter', 'history' )
...
$bm_hero_official = is_array( $bm_history['days'] ?? null ) ? array_slice( $bm_history['days'], -30 ) : array();
...
$bm_hero_source = $bm_hero_available ? trim( (string) ( $bm_snapshot['provenance']['source'] ?? '' ) ) : '';

$bm_hero_default_detail = array( 'date' => '', 'bias' => 'Belum tersedia', 'bias_class' => '', 'certainty' => 'Belum tersedia', 'state' => 'Belum tersedia' );
if ( $bm_hero_official ) {
    $bm_default_record = $bm_hero_official[ count( $bm_hero_official ) - 1 ];
    $bm_default_day = (string) ( $bm_default_record['date'] ?? '' );
    $bm_default_bias = sanitize_key( (string) ( $bm_default_record['directional_bias'] ?? '' ) );
    $bm_default_state = sanitize_key( (string) ( $bm_default_record['market_state'] ?? '' ) );
    $bm_hero_default_detail = array(
        'date'       => $bm_default_day ? wp_date( 'd M', strtotime( $bm_default_day ) ) : '',
        ...
        'certainty'  => $bm_level_bucket( $bm_default_record['market_state_certainty'] ?? 0 ),
        'state'      => class_exists( 'Bitmomo_Regime_Taxonomy' ) && $bm_default_state
            ? Bitmomo_Regime_Taxonomy::regime_label_id( $bm_default_state )
            : ( $bm_default_state ? ucfirst( str_replace( '_', ' ', $bm_default_state ) ) : 'Belum tersedia' ),
    );
}
?>
  ...
  <div class="bm-container bm-hero-layout">
    ...
    <article class="bm-direction-card" aria-label="Kondisi BTC saat ini dan konteks 30 hari">
      ...
      <div class="bm-direction-chart-head">
        <span><?php echo esc_html( count( $bm_hero_official ) ? count( $bm_hero_official ) . ' HARI' : '30 HARI' ); ?></span>
        <span class="bm-direction-chart-key">warna = bias · tinggi = certainty</span>
      </div>

      <div class="bm-direction-chart bm-state-chart" style="--bm-history-count:<?php echo esc_attr( max( 1, count( $bm_hero_official ) ) ); ?>" role="group" aria-label="Riwayat Market State resmi hingga 30 hari. Tinggi batang mengikuti Market State Certainty dan warna mengikuti Directional Bias.">
        <?php if ( $bm_hero_official ) :
          $bm_last_index = count( $bm_hero_official ) - 1;
          foreach ( $bm_hero_official as $bm_index => $bm_record ) :
            $bm_day = (string) ( $bm_record['date'] ?? '' );
            $bm_conf = max( 0, min( 100, (float) ( $bm_record['market_state_certainty'] ?? 0 ) ) );
            $bm_state_key = sanitize_key( (string) ( $bm_record['market_state'] ?? '' ) );
            $bm_state_label = class_exists( 'Bitmomo_Regime_Taxonomy' ) && $bm_state_key ? Bitmomo_Regime_Taxonomy::regime_label_id( $bm_state_key ) : ucfirst( str_replace( '_', ' ', $bm_state_key ) );
            ...
            $bm_bar_date_short = $bm_day ? wp_date( 'd M', strtotime( $bm_day ) ) : '';
            $bm_is_current = ( $bm_index === $bm_last_index );
        ...
        ?>
          <button type="button" class="<?php echo esc_attr( $bm_bar_classes ); ?>" style="--direction-size:<?php echo esc_attr( max( 8, $bm_conf ) ); ?>" title="<?php echo esc_attr( $bm_day . ': ' . $bm_state_label . ' · ' . round( $bm_conf ) . '%' ); ?>" aria-pressed="<?php echo $bm_is_current ? 'true' : 'false'; ?>" aria-label="<?php echo esc_attr( $bm_bar_date_short . ': ' . $bm_bar_bias_label . ', certainty ' . $bm_bar_certainty_label . ', ' . $bm_state_label ); ?>" data-date="<?php echo esc_attr( $bm_bar_date_short ); ?>" data-bias-label="<?php echo esc_attr( $bm_bar_bias_label ); ?>" data-bias-class="<?php echo esc_attr( $bm_bar_bias_valid ? 'is-' . $bm_bar_bias : '' ); ?>" data-certainty-label="<?php echo esc_attr( $bm_bar_certainty_label ); ?>" data-state-label="<?php echo esc_attr( $bm_state_label ); ?>"><span></span></button>
        <?php endforeach; ?>
        <?php else : ?>
      ...
      <?php if ( $bm_hero_official ) : ?>
        <div class="bm-direction-dates" style="--bm-history-count:<?php echo esc_attr( count( $bm_hero_official ) ); ?>" aria-hidden="true">
          <?php foreach ( $bm_hero_official as $bm_record ) : $bm_day = (string) ( $bm_record['date'] ?? '' ); ?>
            <span><?php echo $bm_day ? esc_html( wp_date( 'd', strtotime( $bm_day ) ) ) : ''; ?></span>
          <?php endforeach; ?>
        </div>
      ...
      <footer class="bm-direction-footer">
        ...
        <p><span>DATA</span><?php echo esc_html( $bm_hero_source ?: 'Belum tersedia' ); ?><?php echo $bm_hero_updated ? esc_html( ' · ' . wp_date( 'H:i', $bm_hero_updated ) . ' WIB' ) : ''; ?></p>
      </footer>
...
<?php unset(
  $bm_snapshot, $bm_history, $bm_hero_available, $bm_hero_official, $bm_opportunity, $bm_opportunity_available,
  $bm_opportunity_state, $bm_opportunity_labels, $bm_direction_label, $bm_bias_display_label, $bm_level_bucket,
  $bm_raw_bias, $bm_bias_valid, $bm_latest_strength, $bm_latest_direction, $bm_raw_market_state, $bm_market_state,
  $bm_confidence_map, $bm_confidence_key, $bm_confidence_label, $bm_hero_drivers, $bm_hero_driver, $bm_hero_updated,
  $bm_hero_source, $bm_hero_default_detail, $bm_default_record, $bm_default_day, $bm_default_bias, $bm_default_state,
  $bm_last_index, $bm_index, $bm_record, $bm_day, $bm_conf, $bm_state_key, $bm_state_label, $bm_bar_bias,
  $bm_bar_bias_valid, $bm_bar_bias_label, $bm_bar_certainty_label, $bm_bar_date_short, $bm_is_current, $bm_bar_classes
); ?>
