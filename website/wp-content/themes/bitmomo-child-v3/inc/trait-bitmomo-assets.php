...
trait Bitmomo_Assets_Trait {
    ...
    /**
     * Deterministic CSS ownership graph:
     * parent compatibility -> tokens -> foundation -> shared components ->
     * optional domain component -> exactly one page owner.
     *
     * Legacy custom.css/public-surfaces/navigation-footer files are forbidden
     * from this graph. New fixes must land in their owning layer.
     */
    public function enqueue_styles() {
        ...
        if (is_front_page()) {
            ...
            $this->enqueue_theme_style('bitmomo-research-cards', 'assets/css/components/research-cards.css', ['bitmomo-components']);
            $this->enqueue_theme_style('bitmomo-research', 'assets/css/pages/research.css', ['bitmomo-research-cards']);
        } elseif (is_single()) {
            $this->enqueue_theme_style('bitmomo-research-cards', 'assets/css/components/research-cards.css', ['bitmomo-components']);
            $this->enqueue_theme_style('bitmomo-article', 'assets/css/pages/article.css', ['bitmomo-research-cards']);
        } elseif (is_page('tentang-kami')) {
            $this->enqueue_theme_style('bitmomo-about', 'assets/css/pages/about.css', ['bitmomo-components']);
    ...
    private function is_product_surface_route() {
        return is_page(['pro', 'btc-intelligence', 'help']) || is_page('pro/account');
    }
    ...
    public function remove_bloat() {
        ...
        foreach ($actions as $action) {
            if (3 === count($action)) remove_action($action[0], $action[1], $action[2]);
            else remove_action($action[0], $action[1]);
        }
    }
    ...
    public function filter_loader_src($src) {
        if (!$src) return $src;
        return str_contains($src, 'fonts.googleapis.com') ? add_query_arg('display', 'swap', $src) : $src;
    }

    /** Avoid speculative connections: every external connection must earn its cost. */
