...
final class Bitmomo_AI_Binance {
    ...
    public static function snapshot() {
        $h1 = self::market_klines('1h', 120);
        $h4 = self::market_klines('4h', 320);
        $d1 = self::market_klines('1d', 320);
        ...
        if (count($h1) < 60 || count($h4) < 60 || count($d1) < 60) {
            return new WP_Error('binance_incomplete', __('Binance returned incomplete market data.', 'bitmomo-ai'));
        }
    ...
    private static function market_klines($interval, $limit) {
        $params = ['symbol' => 'BTCUSDT', 'interval' => $interval, 'limit' => $limit];
        $futures = self::get('/fapi/v1/klines', $params);
        if (!is_wp_error($futures)) return $futures;
        return self::get_from_base(self::SPOT_BASE, '/api/v3/klines', $params, 'spot_');
    }
    ...
    private static function get_from_base($base, $path, array $params, $cache_prefix = '') {
        ...
        for ($attempt = 1; $attempt <= 2; $attempt++) {
            ...
            if (is_wp_error($response)) {
                ...
                $last_error = new WP_Error('binance_http', __('Binance market-data request failed.', 'bitmomo-ai'));
            } else {
