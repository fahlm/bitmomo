...
class Bitmomo_Regime_Shortcodes {
	...
	public function body_class( $classes ) {
		global $post;
		if ( is_a( $post, 'WP_Post' ) && ( has_shortcode( $post->post_content, 'bitmomo_market_regime' ) || has_shortcode( $post->post_content, 'bitmomo_market_regime_history' ) ) ) $classes[] = 'bm-regime-page';
		return $classes;
	}
	...
	public function render_current( $atts ) {
		...
		if ( null === $latest ) return $this->style() . '<div class="bmreg-card bmreg-empty">' . esc_html__( 'Market State belum tersedia.', 'bitmomo-regime' ) . '</div>';
		...
		<div class="bmreg-card bmreg-current" data-regime="<?php echo esc_attr( $latest['regime'] ); ?>">
			...
			<div class="bmreg-meta"><span class="bmreg-bias bmreg-bias-<?php echo esc_attr( $bias ); ?>"><?php echo esc_html( ucfirst( $bias ) ); ?></span><span><?php echo esc_html( $conf ); ?>% certainty</span></div>
		</div>
		<?php return $this->style() . trim( (string) ob_get_clean() );
	}
	...
	public function render_history( $atts ) {
		...
		<div class="bmreg-history">
			...
			<?php else : $latest = $days[ count( $days ) - 1 ]; ?>
				<div class="bmreg-history-head"><span><?php echo esc_html( sprintf( '%dD MARKET CONTEXT', count( $days ) ) ); ?></span><div class="bmreg-history-legend" aria-label="Warna menunjukkan Directional Bias"><span class="is-bullish">Bullish</span><span class="is-neutral">Neutral</span><span class="is-bearish">Bearish</span></div></div>
				<div class="bmreg-history-chart" style="--bmreg-count:<?php echo esc_attr( max( 1, count( $days ) ) ); ?>" role="group" aria-label="Riwayat Market State. Tinggi batang menunjukkan certainty dan warna menunjukkan Directional Bias.">
					<div class="bmreg-history-bars">
						<?php foreach ( $days as $index => $day ) :
							...
							$detail = array( 'date'=>(string)($day['date']??''),'regime'=>(string)($day['regime_label_id']??''),'bias'=>ucfirst($bias),'certainty'=>$confidence );
							$is_latest = $index === count( $days ) - 1; ?>
							<button type="button" class="bmreg-history-bar bmreg-bias-<?php echo esc_attr( $bias ); ?><?php echo $is_latest ? ' is-selected' : ''; ?>" style="--bmreg-height:<?php echo esc_attr( $confidence ); ?>%" aria-pressed="<?php echo $is_latest ? 'true' : 'false'; ?>" aria-label="<?php echo esc_attr( $detail['date'] . ': ' . $detail['regime'] . ', ' . $detail['bias'] . ', certainty ' . $confidence . '%' ); ?>" data-bmreg-detail="<?php echo esc_attr( wp_json_encode( $detail ) ); ?>"><span></span></button>
						<?php endforeach; ?>
					</div>
					<div class="bmreg-history-axis" aria-hidden="true"><?php foreach ( $days as $index => $day ) : $show = 0 === $index || $index === count($days)-1 || 0 === ($index % 5); ?><span class="<?php echo $show ? 'is-visible' : ''; ?>"><?php echo $show ? esc_html( wp_date('d M',strtotime($day['date'])) ) : '&nbsp;'; ?></span><?php endforeach; ?></div>
				...
				<?php if ( (int)($full['available_days']??0) < (int)($full['requested_days']??0) ) : ?><p class="bmreg-history-note"><?php echo esc_html( sprintf( __( '%d hari data resmi tersedia sejauh ini.', 'bitmomo-regime' ), (int)($full['available_days']??0) ) ); ?></p><?php endif; ?>
			<?php endif; ?>
		</div>
	...
	private function style() {
		...
		return '<style>'
			...
			. '.bmreg-card{border:1px solid #263449;border-radius:12px;padding:16px;background:#0d1624;color:#e5edf6;}.bmreg-empty{color:#8ea0b8;}'
			. '.bmreg-current-label,.bmreg-history-head>span{display:block;color:#8395ad;font:700 10px/1.3 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;letter-spacing:.12em;}.bmreg-regime-id{display:block;margin-top:5px;font-size:1.2rem;}.bmreg-meta{display:flex;gap:12px;margin-top:8px;color:#8fa1b8;font-size:.82rem;}'
			. '.bmreg-history-head{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:10px;}.bmreg-history-legend{display:flex;align-items:center;gap:12px;color:#8294ae;font-size:10px;}.bmreg-history-legend span{display:inline-flex;align-items:center;gap:5px;}.bmreg-history-legend span:before{content:"";width:6px;height:6px;border-radius:50%;background:#8192aa;}.bmreg-history-legend .is-bullish:before{background:#35cdbb}.bmreg-history-legend .is-bearish:before{background:#ff7b6d}'
			. '.bmreg-history-chart{padding:14px 0 0;border-top:1px solid #1c2a3d;}.bmreg-history-bars{display:grid;grid-template-columns:repeat(var(--bmreg-count),minmax(0,1fr));align-items:end;gap:3px;height:126px;border-bottom:1px solid #263b58;}.bmreg-history-bar{position:relative;width:100%;height:100%;min-width:0;padding:0;border:0;background:transparent;cursor:pointer;}.bmreg-history-bar span{position:absolute;right:0;bottom:0;left:0;height:var(--bmreg-height);min-height:4px;border-radius:3px 3px 1px 1px;background:#8192aa;opacity:.68;transition:opacity .15s ease,filter .15s ease;}'
			. '.bmreg-history-bar.bmreg-bias-bullish span{background:#35cdbb}.bmreg-history-bar.bmreg-bias-bearish span{background:#ff7b6d}.bmreg-history-bar.bmreg-bias-neutral span{background:#8192aa}.bmreg-history-bar:hover span,.bmreg-history-bar:focus-visible span,.bmreg-history-bar.is-selected span{opacity:1;filter:brightness(1.12)}.bmreg-history-bar.is-selected:after{content:"";position:absolute;right:50%;bottom:-6px;width:4px;height:4px;border-radius:50%;background:#e7eef7;transform:translateX(50%);}.bmreg-history-bar:focus-visible{outline:2px solid #dce7f3;outline-offset:2px;border-radius:3px;}'
			. '.bmreg-history-axis{display:grid;grid-template-columns:repeat(var(--bmreg-count),minmax(0,1fr));gap:3px;margin-top:8px;}.bmreg-history-axis span{min-width:0;color:transparent;font-size:9px;text-align:center;white-space:nowrap;}.bmreg-history-axis span.is-visible{color:#8294ae;}.bmreg-history-selected{margin:11px 0 0;color:#a6b5c8;font-size:.82rem;}.bmreg-history-note{margin:9px 0 0;color:#8294ae;font-size:.78rem;}.bmreg-bias-bullish{color:#35cdbb}.bmreg-bias-bearish{color:#ff7b6d}.bmreg-bias-neutral{color:#aebdd2}'
