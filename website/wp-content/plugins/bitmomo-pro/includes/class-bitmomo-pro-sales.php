 ...
 *
 * The buyer journey is proof-first when a real delayed/frozen Decision View is
 * available. If no qualified proof exists, the example surface fails closed
 * instead of rendering an empty or synthetic demonstration. Public rendering
 * never queries the current protected brief.
 */
class Bitmomo_Pro_Sales {
	...
	const SALES_ASSET_VERSION = '2026.09.17-pro-proof-first-v2';
	...
	public function render_sales( $atts ) {
		$proof = $this->delayed_public_proof();
		$has_proof = is_array( $proof );

		ob_start();
		echo '<div class="bm-pro-sales">';

		$this->render_hero( $has_proof );
		$this->render_section_nav( $has_proof );
		if ( $has_proof ) {
			$this->render_product_proof( $proof );
		}
		$this->render_what_exists_today( $has_proof );
	...
	private function render_hero( $has_proof ) {
		?>
		<section id="pro-overview" class="bm-pro-sales__hero" aria-labelledby="bm-pro-hero-title">
			...
			<div class="bm-pro-sales__hero-facts">
				<div><strong>BTC</strong><span><?php esc_html_e( 'fokus produk awal', 'bitmomo-pro' ); ?></span></div>
			...
			<div class="bm-pro-sales__hero-actions">
				...
				<?php else : ?>
					<a class="bm-pro-sales__secondary-cta" href="<?php echo esc_url( home_url( '/btc-intelligence/#decision-ledger' ) ); ?>"><?php esc_html_e( 'Audit rekam jejak publik', 'bitmomo-pro' ); ?></a>
				<?php endif; ?>
			</div>
	...
	private function render_section_nav( $has_proof ) {
		?>
		<nav class="bm-pro-sales__rail" aria-label="<?php esc_attr_e( 'Navigasi Bitmomo Pro', 'bitmomo-pro' ); ?>">
			<?php if ( $has_proof ) : ?><a href="#pro-example"><?php esc_html_e( 'Contoh', 'bitmomo-pro' ); ?></a><?php endif; ?>
	...
	private function render_what_exists_today( $has_proof ) {
		?>
		<section id="pro-product" class="bm-pro-sales__section--editorial bm-pro-sales__today" aria-labelledby="bm-pro-today-title">
			...
			<h2 id="bm-pro-today-title" class="bm-pro-sales__section-title"><?php echo esc_html( $has_proof ? __( 'Decision View historis di atas menunjukkan bentuk produk inti Bitmomo Pro.', 'bitmomo-pro' ) : __( 'Decision View BTC adalah produk inti Bitmomo Pro.', 'bitmomo-pro' ) ); ?></h2>
	...
	private function render_product_proof( $row ) {
		...
		<section id="pro-example" class="bm-pro-sales__product-proof" aria-labelledby="bm-pro-example-title">
			<div class="bm-pro-sales__proof-head">
				<div><p class="bm-pro-sales__eyebrow"><?php esc_html_e( 'PRODUCT PROOF', 'bitmomo-pro' ); ?></p><h2 id="bm-pro-example-title" class="bm-pro-sales__section-title"><?php esc_html_e( 'Lihat bentuk Decision View-nya sebelum membaca daftar fitur.', 'bitmomo-pro' ); ?></h2></div>
	...
	private function render_free_vs_pro() {
		$rows = array(
			...
			array( 'Market State / konteks rezim', 'Ya', 'Ya' ),
		...
		<section class="bm-pro-sales__comparison" aria-labelledby="bm-pro-comparison-title">
			...
			<h2 id="bm-pro-comparison-title" class="bm-pro-sales__section-title"><?php esc_html_e( 'Bedanya bukan lebih banyak data. Bedanya adalah kedalaman analisis.', 'bitmomo-pro' ); ?></h2>
			...
			<p class="bm-pro-sales__micro-note"><?php esc_html_e( 'Gratis sudah cukup untuk memahami kondisi, perubahan, makna, dan satu konteks pantauan. Pro menambahkan monitoring lengkap, Expected Range, skenario, dan invalidasi untuk menavigasi apa yang terjadi berikutnya.', 'bitmomo-pro' ); ?></p>
		</section>
		<?php
	}
	...
	private function render_accountability() {
		...
		<section id="pro-proof" class="bm-pro-sales__accountability" aria-labelledby="bm-pro-accountability-title">
			...
			<div class="bm-pro-sales__accountability-grid">
				<div><strong><?php esc_html_e( 'Dicatat sebelum hasil diketahui', 'bitmomo-pro' ); ?></strong><p><?php esc_html_e( 'Tesis dan konteks dibekukan sebelum hasil diketahui, bukan ditulis ulang setelah pasar bergerak.', 'bitmomo-pro' ); ?></p></div>
	...
	private function render_trust_links() {
		...
		$terms_page = get_page_by_path( 'syarat-layanan', OBJECT, 'page' );
		$terms_url = ( $terms_page && 'publish' === $terms_page->post_status ) ? get_permalink( $terms_page ) : '';
		?>
		<nav class="bm-pro-sales__trust-links" aria-label="<?php esc_attr_e( 'Trust dan kebijakan Bitmomo Pro', 'bitmomo-pro' ); ?>">
			...
			<?php if ( $terms_url ) : ?><a href="<?php echo esc_url( $terms_url ); ?>"><?php esc_html_e( 'Syarat Layanan', 'bitmomo-pro' ); ?></a><?php endif; ?>
		...
		<?php
		unset( $support_email, $terms_page, $terms_url );
	}
