	...
	public static function categories() {
		return array(
			'mulai-di-sini' => array(
				'title' => 'Mulai di Sini',
				'items' => array(
					self::q( 'apa-itu-bitmomo', 'Apa itu Bitmomo?', array( 'Bitmomo adalah platform market intelligence untuk BTC yang merangkum data pasar menjadi kondisi, skenario, dan perubahan yang relevan.', 'Bitmomo bukan layanan pengelolaan dana dan tidak menjanjikan profit.' ) ),
					self::q( 'sinyal-buy-atau-sell', 'Apakah Bitmomo memberikan sinyal beli atau jual?', array( 'Tidak dalam bentuk perintah transaksi. Bitmomo menyediakan dukungan analisis melalui Market State, Bias, Confidence, Expected Range, Scenario Map, kondisi invalidasi tesis, dan perubahan yang relevan.', 'Keputusan investasi tetap berada pada pengguna.' ) ),
				),
			),
			'btc-daily-intelligence' => array(
				'title' => 'BTC Intelligence',
				'items' => array(
					self::q( 'apa-itu-btc-daily-intelligence', 'Apa itu BTC Intelligence?', array( 'BTC Intelligence adalah lapisan gratis Bitmomo untuk memahami kondisi BTC tanpa harus membaca seluruh data mentah.', 'Major Brief menjelaskan kondisi sekarang, perubahan material, mengapa perubahan itu penting, dan satu konteks yang layak dipantau berikutnya.' ) ),
					self::q( 'jadwal-btc-daily-intelligence', 'Seberapa sering BTC Intelligence diperbarui?', array( 'BTC Intelligence memiliki dua clock yang berbeda.', '<strong>Market Pulse</strong> mengevaluasi kondisi intraday setiap 15 menit dari candle 5 menit ketika data memenuhi freshness gate.', '<strong>Major Brief</strong> terbit pada anchor US Post-Close sekitar 20.10 New York dan US Pre-Open sekitar 08.10 New York. Karena anchor mengikuti waktu New York, jam WIB dapat bergeser satu jam ketika daylight-saving AS berubah.', 'Jika data tidak memenuhi standar kualitas atau freshness, analisis saat ini ditahan daripada menampilkan informasi yang sudah tidak layak.' ) ),
					self::q( 'mengapa-dua-kali-sehari', 'Apa fungsi dua Major Brief harian?', array( 'Keduanya menjawab pertanyaan yang berbeda.', 'US Post-Close merangkum apa yang terjadi selama 24 jam terakhir, apa yang berubah, dan mengapa perubahan itu penting.', 'US Pre-Open memusatkan perhatian pada kondisi saat ini menjelang sesi AS, perubahan dibanding brief sebelumnya, dan konteks yang perlu diamati berikutnya.', 'Di antara kedua brief tersebut, Market Pulse menjaga indikator aktivitas intraday tetap lebih cepat tanpa mengubah Major Brief menjadi noise.' ) ),
					self::q( 'quality-gate', 'Apa yang terjadi jika data tidak memenuhi standar kualitas?', array( 'Bitmomo menerapkan standar kualitas data sebelum analisis dapat dipublikasikan.', 'Jika data tidak memenuhi standar, Bitmomo lebih memilih tidak menampilkan current intelligence daripada mengisi kekosongan dengan angka atau analisis yang tidak dapat dipertanggungjawabkan.' ) ),
				),
			),
			'market-state-intelligence' => array(
				...
				'items' => array(
					...
					self::q( 'apa-arti-confidence', 'Apa arti Confidence?', array( 'Confidence menunjukkan seberapa kuat bukti yang mendukung kesimpulan Bitmomo pada saat intelligence dibuat.', 'Confidence yang tinggi bukan jaminan bahwa hasil tertentu akan terjadi.', 'Confidence yang lebih rendah menunjukkan ketidakpastian atau bukti yang saling bertentangan.' ) ),
					self::q( 'market-state-30-hari', 'Apa itu Bias 30 Hari?', array( 'Bias 30 Hari menampilkan riwayat arah BTC yang benar-benar tercatat oleh Bitmomo: Bullish, Netral, atau Bearish.', 'Setiap tanggal hanya memiliki satu Bias resmi dalam riwayat publik sehingga perubahan kecenderungan arah dapat ditelusuri dari waktu ke waktu.' ) ),
					self::q( 'history-belum-30-hari', 'Mengapa riwayat Bias belum selalu berisi 30 hari?', array( 'Bitmomo tidak melakukan backfill retrospektif hanya untuk melengkapi visualisasi.', 'Jika baru tujuh hari yang benar-benar tercatat, Bitmomo akan menampilkan:', '“7 / 30 hari tercatat”', 'Riwayat akan bertambah secara bertahap seiring sistem mencatat Bias baru.' ) ),
				),
			),
			'bitmomo-pro' => array(
				'title' => 'Bitmomo Pro',
				'items' => array(
					self::q( 'apa-itu-bitmomo-pro', 'Apa itu Bitmomo Pro?', array( 'Bitmomo Pro adalah lapisan analisis BTC untuk memahami skenario yang relevan berikutnya dan kapan tesis utama perlu dievaluasi ulang.', 'Pro melengkapi BTC Intelligence gratis dengan pemantauan lengkap, Expected Range, Scenario Map, kondisi invalidasi tesis, dan penjelasan Confidence yang lebih dalam.' ) ),
					self::q( 'free-vs-pro', 'Apa bedanya BTC Intelligence gratis dengan Bitmomo Pro?', array( 'BTC Intelligence gratis sudah cukup untuk memahami kondisi sekarang, perubahan material, mengapa perubahan itu penting, dan satu konteks yang layak dipantau.', 'Bitmomo Pro menambahkan pemantauan lengkap, Expected Range, skenario Base/Bull/Bear, level keputusan, dan kondisi invalidasi untuk membantu memahami apa yang terjadi berikutnya.' ) ),
					...
					self::q( 'apa-itu-thesis-invalidation', 'Apa itu invalidasi tesis?', array( 'Invalidasi tesis adalah kondisi yang membuat tesis utama Bitmomo tidak lagi berlaku.', 'Analisis yang baik menjelaskan skenario yang relevan sekaligus kondisi yang membuat tesis perlu dievaluasi ulang.' ) ),
					self::q( 'apa-itu-what-changed', 'Apa itu “Apa yang Berubah”?', array( 'Bagian ini menunjukkan perubahan penting dibanding analisis sebelumnya.', 'Versi gratis menampilkan perubahan material secara ringkas. Pro menggunakannya bersama skenario, pemantauan lengkap, level, dan invalidasi sehingga perubahan tersebut dapat dibaca dalam konteks yang lebih utuh.' ) ),
				),
			),
			'founding-membership-pricing' => array(
				...
				'items' => array(
					...
					self::q( 'setelah-kuota-149-penuh', 'Apa yang terjadi setelah kuota 149 Founding Members terpenuhi?', array( 'Setelah kuota 149 Founding Members terpenuhi, pendaftaran Founding Membership ditutup untuk member baru.', 'Bitmomo dapat membuka bentuk akses berikutnya di kemudian hari dengan ketentuan atau harga yang berbeda.', 'Founding Members mempertahankan Founding Price selama membership mereka tetap aktif.' ) ),
					self::q( 'member-baru-setelah-penuh', 'Apakah Bitmomo akan menerima member baru setelah Founding Membership penuh?', array( 'Founding Membership tidak menerima member baru setelah kapasitas 149 anggota terpenuhi.', 'Jika akses publik lain dibuka di kemudian hari, ketentuan dan harga dapat berbeda dari Founding Membership.' ) ),
					self::q( 'semua-produk-masa-depan', 'Apakah Founding Members mendapatkan semua produk Bitmomo di masa depan?', array( 'Tidak.', 'Manfaat Founding Membership berlaku untuk fitur baru yang benar-benar diluncurkan sebagai bagian dari Bitmomo Pro selama membership tetap aktif.', 'Produk Bitmomo yang terpisah di masa depan dapat memiliki harga tersendiri.' ) ),
			...
			'ai-lab-decentralized-ai' => array(
				...
				'items' => array(
					...
					self::q( 'mengapa-decentralized-ai', 'Mengapa Bitmomo meneliti AI terdesentralisasi?', array( 'Sistem AI tertutup dapat berubah dari waktu ke waktu—baik dari sisi model, perilaku, harga, maupun kebijakan—tanpa selalu memberi visibilitas penuh terhadap perubahan di balik sistem.', 'Untuk produk analisis yang menuntut konsistensi dan kemampuan audit, ketergantungan pada satu penyedia tertutup menciptakan risiko tersendiri.', 'AI terdesentralisasi menawarkan pendekatan yang berbeda: model dan penyedia dapat dibandingkan lebih terbuka, sementara mekanisme evaluasinya berpotensi dibuat lebih transparan.', 'Bitmomo tidak menganggap arsitektur terdesentralisasi selalu lebih baik. Riset kami menguji kapan pendekatan tersebut benar-benar meningkatkan keandalan dan kemampuan evaluasi sistem.' ) ),
					self::q( 'hubungan-ai-lab-dengan-pro', 'Apa hubungan Riset Sistem AI dengan Bitmomo Pro?', array( 'Riset ini menjadi tempat Bitmomo menguji pendekatan baru sebelum masuk ke produk.', 'Teknologi hanya dipertimbangkan untuk Pro jika terbukti meningkatkan kualitas, konsistensi, transparansi, atau kemampuan evaluasi sistem.' ) ),
				),
			),
			'data-ai-methodology' => array(
				'title' => 'Data, AI & Methodology',
				'items' => array(
					self::q( 'ai-menebak-harga', 'Apakah Bitmomo hanya meminta AI menebak harga Bitcoin?', array( 'Tidak.', 'Bitmomo dibangun sebagai sistem analisis berlapis.', 'Data pasar terlebih dahulu dinormalisasi, diperiksa kualitasnya, dan diproses dengan aturan deterministik sebelum digunakan untuk klasifikasi Market State atau analisis lanjutan.', 'AI tidak diposisikan sebagai mesin yang selalu benar.' ) ),
					self::q( 'deterministic-dan-ai', 'Kenapa Bitmomo menggunakan aturan deterministik sekaligus AI?', array( 'Keduanya memiliki kelebihan yang berbeda.', 'Aturan deterministik membantu menjaga konsistensi, keterulangan hasil, dan batas yang jelas.', 'AI lebih berguna untuk interpretasi, penalaran, dan sintesis informasi yang lebih kompleks.', 'Bitmomo menggabungkan keduanya agar proses analisis tidak bergantung pada satu model AI tertutup.' ) ),
				),
			),
		);
	}
