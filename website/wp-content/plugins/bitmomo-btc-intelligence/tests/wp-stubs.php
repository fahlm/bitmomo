...
$GLOBALS['__wp_stub_shortcodes'] = array();
function add_shortcode( $tag, $callback ) {
	$GLOBALS['__wp_stub_shortcodes'][ $tag ] = $callback;
}
function shortcode_exists( $tag ) {
	return isset( $GLOBALS['__wp_stub_shortcodes'][ $tag ] );
}
function do_shortcode( $content ) {
	// Minimal support for the single-tag, no-nesting case this test suite
	// actually exercises: '[tag]' or '[tag attr="val" ...]'.
	return preg_replace_callback(
		'/\[([a-zA-Z0-9_\-]+)([^\]]*)\]/',
		static function ( $m ) {
			$tag = $m[1];
			if ( ! isset( $GLOBALS['__wp_stub_shortcodes'][ $tag ] ) ) {
				return $m[0];
			}
			$atts = array();
			if ( preg_match_all( '/([a-zA-Z0-9_\-]+)="([^"]*)"/', $m[2], $am, PREG_SET_ORDER ) ) {
				foreach ( $am as $pair ) {
					$atts[ $pair[1] ] = $pair[2];
				}
			}
			return (string) call_user_func( $GLOBALS['__wp_stub_shortcodes'][ $tag ], $atts );
		},
		$content
	);
}
...
function get_page_by_path( $path, $output = OBJECT, $post_type = 'page' ) {
	foreach ( $GLOBALS['__wp_stub_posts'] ?? array() as $id => $post ) {
		if ( ( $post['post_type'] ?? '' ) === $post_type && ( $post['post_name'] ?? '' ) === $path ) {
			return (object) array_merge( array( 'ID' => $id ), $post );
		}
	}
	return null;
}
...
function date_i18n( $format, $timestamp = false ) {
	return date( $format, false === $timestamp ? time() : $timestamp );
}
