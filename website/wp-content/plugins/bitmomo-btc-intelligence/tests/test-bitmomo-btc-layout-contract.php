...
layout_check( 'Desktop/tablet rail offset follows canonical site header token with 64px fallback', false !== strpos( $css, '--bmi-sticky-header:var(--bm-header-height,64px)' ) );
layout_check( 'Rail height has one explicit clearance token', false !== strpos( $css, '--bmi-sticky-rail:63px' ) );
layout_check( 'Mobile rail offset follows canonical mobile header token with 60px fallback', preg_match( '/@media\(max-width:768px\)\{\.bm-bi\{--bmi-sticky-header:var\(--bm-header-height-mobile,60px\)\}\}/', $css ) === 1 );
